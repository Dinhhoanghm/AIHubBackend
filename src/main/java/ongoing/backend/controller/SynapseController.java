...
public class SynapseController {
  @Autowired
  private SynapseDataSource synapseDataSource;
  ...
  public ResponseEntity<Map<String, Object>> getAllData(@RequestBody Map<String, Object> data) {
    ...
    try {
      ...
      if (StringUtils.isBlank(params)) {
        cart.putAll(synapseDataSource.getData(data));
      } else {
        Map<String, Object> mapParams = new JsonObject(params).getMap();
        cart.putAll(synapseDataSource.getData(mapParams));
      }
      if (cart.isEmpty()) {
  ...
  public ResponseEntity<Map<String, Object>> getAllMetadata(@RequestBody Map<String, Object> data) {
    try {
      ...
      String params = data.getOrDefault("query", "").toString();
      if (StringUtils.isBlank(params)) {
        cart.putAll(synapseDataSource.getMetaData(data));
      } else {
        Map<String, Object> mapParams = new JsonObject(params).getMap();
        cart.putAll(synapseDataSource.getMetaData(mapParams));
      }
      if (cart.isEmpty()) {
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
      }
  ...
  public ResponseEntity<Map<String, Object>> getConnection(@RequestBody Map<String, Object> data) {
    try {
      return new ResponseEntity<>(synapseDataSource.testConnection(data), HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  public ResponseEntity<Map<String, Object>> getObjects(@RequestBody Map<String, Object> data) {
    try {
      Map<String, Object> response = synapseDataSource.getObjects(data);
